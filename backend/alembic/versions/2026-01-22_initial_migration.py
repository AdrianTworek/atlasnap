...
Revision ID: be6c7b09111e
Revises:
Create Date: 2026-01-22 13:42:33.442934
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["user.id"],
            name=op.f("oauth_account_user_id_fkey"),
            ondelete="cascade",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("oauth_account_pkey")),
    )
    op.create_index(
        op.f("oauth_account_account_id_idx"),
        "oauth_account",
        ["account_id"],
        unique=False,
    )
    op.create_index(
        op.f("oauth_account_oauth_name_idx"),
        "oauth_account",
        ["oauth_name"],
        unique=False,
    )
